...
RUN apk update && \
    ...
    apk add --no-cache wget unzip git curl gcc musl-dev python3-dev libffi-dev aws-cli rpm2cpio chromium chromium-chromedriver openjdk11 xmlstarlet jq dos2unix && \
...
RUN dos2unix main_executor.sh && chmod +x main_executor.sh
