...
aws s3 cp --acl bucket-owner-full-control --recursive s3://tf-rf-scripts-spe-qaqc-bucket/SPT_VRT/aut-vrt-spt/ ./backstopJS
if [ $? -ne 0 ]; then
...
for folder in /vrt/backstopJS/*/; do
    ...
    ## RUN tests
    backstop test --config="$folder/backstop.json"
    if [ $? -ne 0 ]; then
